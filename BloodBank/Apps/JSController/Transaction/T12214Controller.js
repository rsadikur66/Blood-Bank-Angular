app.controller("T12214Controller", ["$scope", "T12214Service", "$filter", "$http", "$window", "Data",
    function ($scope, T12214Service, $filter, $http, $window, Data) { //$location,
        ...
        $scope.ExecuteButton = function () {
            var pat =$scope.obj.t03001.T_PAT_NO;
            var nat = $scope.obj.t03001.T_NTNLTY_ID;
            var fisName = '';//$scope.obj.t03001.T_FIRST_LANG2_NAME.toUpperCase();
            var fathName = '';//$scope.obj.t03001.T_FATHER_LANG2_NAME.toUpperCase();
            var graName = '';//$scope.obj.t03001.T_GFATHER_LANG2_NAME.toUpperCase();
            var famName = '';// $scope.obj.t03001.T_FAMILY_LANG2_NAME.toUpperCase();
            var year = $scope.obj.t03001.YRS;
            var gendr = $scope.obj.G.selected.T_SEX_CODE;
            var natnl = $scope.obj.N.selected.T_NTNLTY_CODE;
            var regn = $scope.obj.RG.selected.T_RLGN_CODE;
            var mrists = $scope.obj.M.selected.T_MRTL_STATUS_CODE;
           
            if (pat === '' && nat === '' && fisName === '' && fathName === '' && graName === '' && famName === ''
                && gendr === undefined && natnl === undefined && regn === undefined && mrists === undefined && year === '') {
                ...
                document.getElementById("ddlIGenderId").readOnly = false;  
                document.getElementById("ddlINationalityId").readOnly = false;  
                document.getElementById("ddlIReligionId").readOnly = false;  
                document.getElementById("ddlIMaritialStatusId").readOnly = false;
                ...
                //$scope.clearAllField();
                Execute(pat, nat, fisName, fathName, graName, famName, gendr, natnl, regn, mrists, year);
                document.getElementById("txtNationalId").focus();
            }
        ...
        $scope.keyDown = function (e) {
            var retrievedData = localStorage.getItem("newDataList");
            var newDataList = JSON.parse(retrievedData);
            if (e.keyCode === 40) {
                ...
                if (i === m) {
                    ...
                    var lang = $scope.lang;
                    $scope.obj.t03001.T_NTNLTY_ID = newDataList[i].T_NTNLTY_ID;
                    $scope.obj.t03001.T_PAT_NO = newDataList[i].T_PAT_NO;
                    $scope.obj.t03001.T_FIRST_LANG2_NAME = newDataList[i].T_FIRST_LANG2_NAME;
                    $scope.obj.t03001.T_FIRST_LANG1_NAME = newDataList[i].T_FIRST_LANG1_NAME;
                    $scope.obj.t03001.T_FATHER_LANG1_NAME = newDataList[i].T_FATHER_LANG1_NAME;
                    $scope.obj.t03001.T_FATHER_LANG2_NAME = newDataList[i].T_FATHER_LANG2_NAME;
                    $scope.obj.t03001.T_FAMILY_LANG1_NAME = newDataList[i].T_FAMILY_LANG1_NAME;
                    $scope.obj.t03001.T_FAMILY_LANG2_NAME = newDataList[i].T_FAMILY_LANG2_NAME;
                    $scope.obj.t03001.T_GFATHER_LANG1_NAME = newDataList[i].T_GFATHER_LANG1_NAME;
                    $scope.obj.t03001.T_GFATHER_LANG2_NAME = newDataList[i].T_GFATHER_LANG2_NAME;
                    $scope.obj.t03001.T_MOTHER_LANG1_NAME = newDataList[i].T_MOTHER_LANG1_NAME;
                    $scope.obj.t03001.T_MOTHER_LANG2_NAME = newDataList[i].T_MOTHER_LANG2_NAME;
                    $scope.obj.t03001.T_PAT_TITLE = newDataList[i].T_PAT_TITLE;
                    $scope.obj.t03001.T_SEX_CODE = newDataList[i].T_GENDER;
                    $scope.obj.t03001.T_NTNLTY_CODE = newDataList[i].T_NTNLTY_CODE;
                    $scope.obj.t03001.T_RLGN_CODE = newDataList[i].T_RLGN_CODE;
                    $scope.obj.t03001.T_MRTL_STATUS_CODE = newDataList[i].T_MRTL_STATUS_CODE;
                    $scope.obj.t03001.T_RLTN_CODE = newDataList[i].T_RLTN_CODE;
                    $scope.obj.t03001.RegisterBy = newDataList[i].T_ENTRY_USER_NAME;
                    var a = Date.parse(newDataList[i].T_ENTRY_DATE);
                    var b = $filter('date')(a, "dd-MMM-yyyy");
                    $scope.obj.t03001.T_ENTRY_DATEARB = moment(b).format('iYYYY/iMM/iDD');
                    $scope.obj.t03001.ModifiedBy = newDataList[i].T_UPDATE_USER_NAME;
                    $scope.obj.t03001.T_UPD_DATE = newDataList[i].T_UPD_DATE;
                    if ($scope.obj.t03001.T_UPD_DATE != null) {
                        var c = Date.parse(newDataList[i].T_UPD_DATE);
                        var u = $filter('date')(c, "dd-MMM-yyyy");
                        $scope.obj.t03001.T_UPD_DATEARB = moment(u).format('iYYYY/iMM/iDD');
                    } else {
                        $scope.obj.t03001.T_UPD_DATE = '';
                    }
                    $scope.obj.t03001.YRS = newDataList[i].YRS;
                    $scope.obj.t03001.MOS = newDataList[i].MOS;

                    var mydate = Date.parse(newDataList[i].T_BIRTH_DATE);
                    if (mydate != undefined) {
                    ...
                    if (newDataList[i].T_ENTRY_DATE != null) {
                        var myEntryDate = Date.parse(newDataList[i].T_ENTRY_DATE);
                        $scope.obj.t03001.T_ENTRY_DATE = $filter('date')(myEntryDate, "dd/MM/yyyy");
                    } else {
                        newDataList[i].T_ENTRY_DATE = '';
                    }
                    if (newDataList[i].T_UPD_DATE != null) {
                        var myUpdateDate = Date.parse(newDataList[i].T_UPD_DATE);
                        $scope.obj.t03001.T_UPD_DATE = $filter('date')(myUpdateDate, "dd/MM/yyyy");
                    } else {
                        newDataList[i].T_UPD_DATE = '';
                    }


                    if (lang != '1') {
                        $scope.obj.t03001.T_ADDRESS1 = newDataList[i].T_ADDRESS1;
                    } else {
                        $scope.obj.t03001.T_ADDRESS2 = newDataList[i].T_ADDRESS2;
                    }
                    $scope.obj.t03001.T_MOBILE_NO = newDataList[i].T_MOBILE_NO;


                    //DropdownList
                    $scope.obj.G.selected = { T_GENDER: newDataList[i].T_GENDER_NAME, T_SEX_CODE: newDataList[i].T_GENDER };
                    $scope.obj.t03001.T_GENDER = newDataList[i].T_GENDER;
                    $scope.obj.T.selected = { T_TITLE_NAME: newDataList[i].T_PAT_TITLE_NAME, T_TITLE_CODE: newDataList[i].T_PAT_TITLE };
                    $scope.obj.t03001.T_TITLE_CODE = newDataList[i].T_PAT_TITLE;
                    $scope.obj.N.selected = { T_NTNLTY: newDataList[i].T_NTNLTY_NAME, T_NTNLTY_CODE: newDataList[i].T_NTNLTY_CODE };
                    $scope.obj.RG.selected = { T_RLGN_NAME: newDataList[i].T_RLGN_NAME, T_RLGN_CODE: newDataList[i].T_RLGN_CODE };
                    $scope.obj.M.selected = { T_MRTL_STATUS: newDataList[i].T_MRTL_STATUS_NAME, T_MRTL_STATUS_CODE: newDataList[i].T_MRTL_STATUS };
                    $scope.obj.t03001.T_MRTL_STATUS = newDataList[i].T_MRTL_STATUS;
                    $scope.obj.ER.selected = { T_ERRLTN_NAME: newDataList[i].T_ERRLTN_NAME, T_ER_RLTN_CODE: newDataList[i].T_ER_RLTN_CODE };
                    $scope.obj.t03001.T_ER_RLTN_CODE = newDataList[i].T_ER_RLTN_CODE;
                    $scope.obj.RT.selected = { T_RLTN_NAME: newDataList[i].T_RLTN_NAME, T_RLTN_CODE: newDataList[i].T_RLTN_CODE };
                    $scope.obj.t03001.T_RLGN_CODE = newDataList[i].T_RLGN_CODE;
                           ...
                           // $scope.nationalIdCheck();
                            nationlityCode.then(function (data) {
                                ...
                                if (newDataJSON[0] != undefined) {
                                    $scope.clearAllField();                                    
                               // Execute(pat, newDataJSON[0].T_NTNLTY_ID, fisName, fathName, graName, famName, gendr, natnl, regn, mrists, year);
                                   
                                   Execute('', newDataJSON[0].T_NTNLTY_ID);
                                } else {
            ...
            if (event.keyCode === 13) {
                ...
                if (controlkeyName === "txtFatherName") {
                    if ($scope.obj.t03001.T_FATHER_LANG2_NAME != '') {
                        $scope.checkIslamReligionfromName($scope.obj.t03001.T_FATHER_LANG2_NAME);
                        $scope.getFatherNameArb($scope.obj.t03001.T_FATHER_LANG2_NAME);
                    }
                }
                ...
                if (controlkeyName === "txtGrandFatherName") {
                    $scope.getGrandFatherNameArb($scope.obj.t03001.T_GFATHER_LANG2_NAME);
                    $scope.checkIslamReligionfromName($scope.obj.t03001.T_GFATHER_LANG2_NAME);
                ...
                if (controlkeyName === "txtMotherName") {
                    $scope.getMotherNameArb($scope.obj.t03001.T_MOTHER_LANG2_NAME);
        ...
        $scope.getFirstNameArb = function (englishName) {
            $scope.obj.t03001.T_FIRST_LANG1_NAME = '';
            $scope.obj.t03001.T_FIRST_LANG2_NAME = englishName;
            var NameArb = T12214Service.getArabicName(englishName);
            NameArb.then(function (data) {
                var newDataJSON = JSON.parse(data);
                if (newDataJSON[0].T_NAME_ARB != undefined) {
                    $scope.obj.t03001.T_FIRST_LANG1_NAME = newDataJSON[0].T_NAME_ARB;
                    document.getElementById("txtFatherName").focus();
                }
            });
        ...
        $scope.getFatherNameArb = function (englishName) {
            $scope.obj.t03001.T_FATHER_LANG1_NAME = '';
            $scope.obj.t03001.T_FATHER_LANG2_NAME = englishName;
            var NameArb = T12214Service.getArabicName(englishName);
            NameArb.then(function (data) {
                var newDataJSON = JSON.parse(data);
                if (newDataJSON[0].T_NAME_ARB != undefined) {
                    $scope.obj.t03001.T_FATHER_LANG1_NAME = newDataJSON[0].T_NAME_ARB;
                    document.getElementById("txtGrandFatherName").focus();
                }
            });
            if ($scope.T_NAME_ARB == undefined) {
        ...
        $scope.getGrandFatherNameArb = function (englishName) {
            $scope.obj.t03001.T_GFATHER_LANG1_NAME = '';
            $scope.obj.t03001.T_GFATHER_LANG2_NAME = englishName;
            var NameArb = T12214Service.getArabicName(englishName);
            NameArb.then(function (data) {
                var newDataJSON = JSON.parse(data);
                if (newDataJSON[0].T_NAME_ARB != undefined) {
                    $scope.obj.t03001.T_GFATHER_LANG1_NAME = newDataJSON[0].T_NAME_ARB;
                    document.getElementById("txtLastName").focus();
                }
            });
            if ($scope.T_NAME_ARB == undefined) {
        ...
        $scope.getFamilyNameArb = function (englishName) {
            $scope.obj.t03001.T_FAMILY_LANG1_NAME = '';
            $scope.obj.t03001.T_FAMILY_LANG2_NAME = englishName;
            var NameArb = T12214Service.getArabicName(englishName);
            NameArb.then(function (data) {
                var newDataJSON = JSON.parse(data);
                if (newDataJSON[0].T_NAME_ARB != undefined) {
                    $scope.obj.t03001.T_FAMILY_LANG1_NAME = newDataJSON[0].T_NAME_ARB;
                    document.getElementById("txtBirthDateEng").focus();
                }
            });
            if ($scope.T_NAME_ARB == undefined) {
        ...
        $scope.getMotherNameArb = function (englishName) {
            $scope.obj.t03001.T_MOTHER_LANG1_NAME = '';
            $scope.obj.t03001.T_MOTHER_LANG2_NAME = englishName;
            var NameArb = T12214Service.getArabicName(englishName);
            NameArb.then(function (data) {
                var newDataJSON = JSON.parse(data);
                if (newDataJSON[0].T_NAME_ARB != undefined) {
                    $scope.obj.t03001.T_MOTHER_LANG1_NAME = newDataJSON[0].T_NAME_ARB;
                    document.getElementById("txtFamHead").focus();
                }
            });
            if ($scope.T_NAME_ARB == undefined) {
        ...
        $scope.getTitleList = function (ddlTitleId) {
            ...
            $scope.ddlfocusLost('txtFirstName');
        }
        ...
        $scope.newDataList = [];
         function Execute (pat, nat, fisName, fathName, graName, famName, gendr, natnl, regn, mrists, year) {
            //$scope.obj.t03001.T_PAT_NO = T_PAT_NO;
            //$scope.obj.t03001.T_NTNLTY_ID = T_NTNLTY_ID;
            var patdata = T12214Service.getExistingPatData(pat, nat, fisName, fathName, graName, famName, gendr, natnl, regn, mrists, year);
            patdata.then(function (data) {
                var newDataJSON = JSON.parse(data);
                $scope.newDataList = newDataJSON;
                localStorage.setItem("newDataList", JSON.stringify(newDataJSON));
                if (newDataJSON[0] == undefined) {
                ...
                $scope.obj.T.selected = { T_TITLE_NAME: newDataJSON[0].T_PAT_TITLE_NAME, T_TITLE_CODE: newDataJSON[0].T_PAT_TITLE };
                $scope.obj.t03001.T_TITLE_CODE = newDataJSON[0].T_PAT_TITLE;
                $scope.obj.N.selected = { T_NTNLTY: newDataJSON[0].T_NTNLTY_NAME, T_NTNLTY_CODE: newDataJSON[0].T_NTNLTY_CODE };
                $scope.obj.RG.selected = { T_RLGN_NAME: newDataJSON[0].T_RLGN_NAME, T_RLGN_CODE: newDataJSON[0].T_RLGN_CODE };
                $scope.obj.M.selected = { T_MRTL_STATUS: newDataJSON[0].T_MRTL_STATUS_NAME, T_MRTL_STATUS_CODE: newDataJSON[0].T_MRTL_STATUS };
                $scope.obj.t03001.T_MRTL_STATUS = newDataJSON[0].T_MRTL_STATUS;
                $scope.obj.ER.selected = { T_ERRLTN_NAME: newDataJSON[0].T_ERRLTN_NAME, T_ER_RLTN_CODE: newDataJSON[0].T_ER_RLTN_CODE };
                $scope.obj.t03001.T_ER_RLTN_CODE = newDataJSON[0].T_ER_RLTN_CODE;
                $scope.obj.RT.selected = { T_RLTN_NAME: newDataJSON[0].T_RLTN_NAME, T_RLTN_CODE: newDataJSON[0].T_RLTN_CODE };
                $scope.obj.t03001.T_RLGN_CODE = newDataJSON[0].T_RLGN_CODE;
                document.getElementById("btnEnterQuery").className = "Button Enter";
                document.getElementById("txtDonarId").readOnly = true;
                sessionStorage.setItem("PatNo", JSON.stringify($scope.obj.t03001.T_PAT_NO));
